...
class TestRemoteFTP(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.credentials = {
            ...
            'passive_mode': os.environ.get('FTP_PASSIVE_MODE', True),
    ...
    def test_ftp_search_path_ext(self):
        files = self.driver_ftp.with_path(os.environ.get('FTP_DIR')).with_mask('*.xml').search()
        print(files)
        for file in files:
            self.assertIsInstance(file, RemoteFile)
            self.assertIsNotNone(file.name)
    ...
    def test_ftp_search_path_mask(self):
        files = self.driver_ftp.with_path(os.environ.get('FTP_DIR')).with_mask('*OFF*').search()
        for file in files:
            self.assertIsInstance(file, RemoteFile)
            self.assertIsNotNone(file.name)
            self.assertEqual(len(file.content), 0)
    ...
    def test_ftp_download_path(self):
        files = self.driver_ftp.with_path(os.environ.get('FTP_DIR')).with_mask('*TAKEOFF*').download()
    ...
    def test_ftp_upload_file(self):
        ...
        res = files.with_driver(self.driver_ftp).with_path(os.environ.get('FTP_DIR')).upload()
